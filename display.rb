...
class Display
  ...
  def initialize(board)
    ...
    start_color
    init_pair(COLOR_RED, COLOR_RED, COLOR_CYAN)
  ...
  def render
    (0..7).each do |i|
      (0..7).each do |j|
        ...
        if @cursor_pos == [i,j]
          place_string(i, j*3," #{piece.to_s} ", COLOR_RED)
        else
          if (i + j) % 2 == 0
            place_string(i, j*3," #{piece.to_s} ", COLOR_WHITE)
          else
            place_string(i, j*3," #{piece.to_s} ", COLOR_BLACK)
          end
        end
      end
    end
